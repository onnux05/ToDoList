...
let onnux = 'mongodb+srv://onnux:Murugan@9@cluster0.obhn1.mongodb.net/onnux_db?retryWrites=true&w=majority'
let dbName = 'onnux-db'
...
MongoClient.connect(onnux,{useNewUrlParser:true,useUnifiedTopology: true },function(err,client){
  if(err){
    throw err
}
      db=client.db(dbName)
...
app.use(express.json())
  app.use(express.urlencoded({extended:false}))
    ...
    function passProtected(req,res,next){
      if(req.headers.authorization == 'Basic b25udXg6c2FyYXZhbmFuOQ=='){
          next()
    }else{
